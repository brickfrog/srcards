...
const onToggleUserCard = function () {
  if (get("user-data-card").style.visibility == "visible") {
    ...
    get("user-data-card-text").innerHTML = `
        ...
        You have flipped through <b>${userData.countCardsSeen}</b> cards (<b>${
      Object.keys(userData.prioForHash).length
    }</b> unique).<br>
...
const stopDragging = function () {
  ...
  const dropBox = document.elementFromPoint(
    lastMoveCoordinates.x,
    lastMoveCoordinates.y
  );
...
const onMouseMove = function (event) {
  ...
  if (mouseDragging || Math.abs(diffX) + Math.abs(diffY) > 20) {
    ...
    ).style.transform = `translate(${diffX}px, ${diffY}px)`;
...
document.addEventListener("touchend", (e) => {
  ...
  if (
    e.target &&
    (e.target.id === "card-right" || e.target.kid === "card-left")
  ) {
...
const saveStateToLocalStorage = function () {
  ...
  if (
    sessionIdForDuplicateDetection !=
    localStorage.getItem(LOCAL_STORAGE_DUPLICATE_DETECTION_KEY)
  ) {
    alert(
...
const initializeUserSession = function () {
  ...
  localStorage.setItem(
    LOCAL_STORAGE_DUPLICATE_DETECTION_KEY,
    sessionIdForDuplicateDetection
  );
  ...
  ["aws", "azure", "gcp"].forEach((deck) => {
    ...
    if (
      minPrioInUserData - PRIO_INITIAL > PRIO_INCREASE_LATER &&
      minPrioInUserData < PRIO_INCREASE_NEVER
    ) {
      for (let i = 0; i < flipcards.length; i++) {
        const c = flipcards[i];
        if (c.deck != deck) {
...
const throwAwayCard = function (translateX, translateY) {
  get(
    "flip-card-outer-container"
  ).style.transform = `translate(${translateX}px, ${translateY}px) scale(0)`;
};
...
const changeCurrentCard = function () {
  ...
  if (!minCard) {
    ...
    if (get("deck-filter-input").value != "") {
      minCard.q += ` You are currently filtering the deck for '${
        get("deck-filter-input").value
...
const updateStreak = function (latestVal) {
  ...
  userData.last100streak = userData.last100streak.slice(
    Math.max(0, userData.last100streak.length - 30)
  );
};
